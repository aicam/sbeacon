...
export default class Search extends React.Component {
    ...
    componentDidMount() {
        fetch('http://parsbeacon.ir/requests/search_data?indexstr=' + this.props.indexstr + '&offset=0').then(
            (response) => {
    ...
    fetch_new_data() {
        ...
        fetch('http://parsbeacon.ir/requests/search_data?indexstr=' + this.props.indexstr + '&offset=' + this.state.offset).then(
            (response) => {
                response.json().then((jsondata) => {
                        let new_data = this.state.items;
                        if(jsondata.items.length) {
                            let new_data = this.state.items.concat(jsondata.items);
                            this.setState({
                                data: new_data
                            })
                        }
